...
int main(int argc, const char **argv)
{
    ...
    {
        syslog(LOG_ERR, "%d (%s) registering for SIGTERM", errno, strerror(errno));
    }
    ...
    {
        syslog(LOG_ERR, "Error creating socket %d (%s)", errno, strerror(errno));
        exit(1);
    }
    const int enable = 1;
    ...
    while (!quit)
    {
        ...
        while (ret == NULL)
        {
            ...
            if (readSize == -1)
            {
                syslog(LOG_ERR, "Error reading data from client: %s", strerror(errno));
                break;
            }
            if (readSize == 0)
                break;
        ...
        if (fwrite(buffer, 1, totalSize, fp) != totalSize)
        {
            syslog(LOG_ERR, "Error writing to file aesdsocketedata: %s", strerror(errno));
        }
